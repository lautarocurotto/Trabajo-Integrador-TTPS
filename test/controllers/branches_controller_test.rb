...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference("Branch.count") do
      post branches_url, params: { branch: { address: @branch.address, name: @branch.name, phone: @branch.phone } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { address: @branch.address, name: @branch.name, phone: @branch.phone } }
